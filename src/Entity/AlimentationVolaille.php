...
use App\Repository\AlimentationVolailleRepository;
use Doctrine\DBAL\Types\Types;
use Doctrine\ORM\Mapping as ORM;

#[ORM\Entity(repositoryClass: AlimentationVolailleRepository::class)]
#[ORM\Table(name: 'alimentations_volaille')]
class AlimentationVolaille
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    #[ORM\Column]
    ...
    #[ORM\ManyToOne(targetEntity: Volaille::class, inversedBy: 'alimentations')]
    #[ORM\JoinColumn(nullable: false)]
    private ?Volaille $volaille = null;

    #[ORM\ManyToOne(targetEntity: Stock::class, inversedBy: 'alimentationsVolaille')]
    #[ORM\JoinColumn(nullable: false)]
    ...
    public function calculerQuantiteParTete(): ?string
    {
        if (!$this->effectifLors || $this->effectifLors == 0) {
            return null;
        }

        return bcdiv($this->quantite, $this->effectifLors, 4);
    }

    public function calculerCoutParTete(): ?string
    {
        $coutTotal = $this->calculerCout();
        if (!$coutTotal || !$this->effectifLors || $this->effectifLors == 0) {
            return null;
        }

        return bcdiv($coutTotal, $this->effectifLors, 4);
    }
    ...
    {
        return $this->volaille?->getNumeroLot() . ' - ' . $this->stock?->getNom() . ' - ' . $this->date->format('d/m/Y');
    }
}